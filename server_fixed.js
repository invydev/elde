...
// Konfigurasi (ubah nama menjadi appConfig untuk menghindari overwrite)
const appConfig = {
  egg: "1",
  nestid: "5",
  loc: "1",
...
app.post('/order', async (req, res) => {
  ...
  try {
    ...
    const depositRes = await fetch(`${appConfig.atlantic}/deposit/create`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({
        api_key: appConfig.apikeyh2h,
        reff_id: reffId,
        nominal: amount,
        type: 'ewallet',
        metode: 'QRISFAST'
...
app.get('/check-order', async (req, res) => {
  ...
  try {
    ...
    const statusRes = await fetch(`${appConfig.atlantic}/deposit/status`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ 
        api_key: appConfig.apikeyh2h, 
...
async function createPanel(username, ram) {
  ...
  const serverConfig = serverConfigs[ram];
  if (!serverConfig) throw new Error('Invalid server configuration');
  ...
  const userRes = await fetch(`${appConfig.domain}/api/application/users`, {
    method: "POST",
    headers: {
  ...
  const eggRes = await fetch(
    `${appConfig.domain}/api/application/nests/${appConfig.nestid}/eggs/${appConfig.egg}`, 
    {
      method: "GET",
      headers: {
        "Accept": "application/json",
  ...
  // Buat server
  const serverRes = await fetch(`${appConfig.domain}/api/application/servers`, {
    method: "POST",
    headers: {
      "Accept": "application/json",
      "Content-Type": "application/json",
    ...
    body: JSON.stringify({
      ...
      deploy: {
        locations: [parseInt(appConfig.loc)],
        dedicated_ip: false,
        port_range: [],
      }
    })
